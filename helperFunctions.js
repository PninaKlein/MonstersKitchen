function findLikes(foodItem){
    options=[];
    // iterate through all preferance options
    Object.keys(food_list).forEach(food=>{
        let att= food_list[food]["attributes"];
        let preferance= {"type": att["type"], "color": att["color"], "size": att["size"]};
        // calculate which items are possible likes in next round
        let round1_possibleLikes= calculate_next_round(foodItem.name, preferance, food_list);
        ...
        Object.keys(round1_possibleLikes).forEach(item=>{
            let round2_possibleLikes= calculate_next_round(item, preferance, round1_possibleLikes);
        ...
        options.push({"variability": for_calc.length, "preferance": preferance})
        console.log(options)
    })
    let max_index= 0;
    let max_val= 0;
    ...
    //console.log(options)
    return options[max_index]["preferance"];
}

function calculate_next_round(foodItem, preferance, food_list){
    ...
    Object.keys(food_list).forEach(item=>{
        ...
        if(chosen_size!= preferance["size"] && chosen_color!= preferance["color"] && chosen_type!= preferance["type"]){ 
        ...
        else if(chosen_size== preferance["size"] && chosen_color== preferance["color"] && chosen_type== preferance["type"]){
